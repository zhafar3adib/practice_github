def mapper(lines):
    ...
    if isinstance(lines, str):
        words = lines.split(' ')
        # words = ["word", "count", "from", "Wikipedia", "the", "free", "encyclopedia", ...]
        for word in words:
            mapped.append((word.lower(), 1))
            # mapped = [("word", 1), ("count", 1), ("from", 1), ("wikipedia", 1), ...]
    return mapped

def shuffle(mapped):
    groups = {}
    for word, count in mapped:
        groups.setdefault(word, [])
        groups[word].append(count)
        # groups = {"word": [1], "count": [1], "from": [1], "wikipedia": [1], ...}
    return groups
...
def reducer(groups):
    ...
    for word, counts in groups.items():
        ...
        # reduced = {"word": 1, "count": 1, "from": 1, "wikipedia": 1, ...}
    return reduced

def print_result(wordcount):
    sorted_wordcount = sorted(wordcount.items(), key=lambda x: x[1], reverse=True)
    for word, count in sorted_wordcount:
        print(f"Word: {word}, Count: {count}")

def main():
    sample_text = """word count from Wikipedia the free encyclopedia
...
Numerous American universities limit Ph.D. dissertations to at most words barring special permission for exceeding this limit"""
    ...
    remove_newline = sample_text.replace("\n", " ")

    mapped = mapper(remove_newline)
    groups = shuffle(mapped)
    wordcount = reducer(groups)

    print_result(wordcount)
